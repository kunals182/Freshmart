    ...
    return (
        <div className="bg-white rounded-2xl p-6 animate-fade-in">
            ...
            <div className="mt-6 flex flex-col sm:flex-row items-center gap-3">
                <Link to="/addresses" className="w-full sm:w-auto px-6 py-3 border border-app-green/20 text-app-green font-semibold rounded-xl flex-center gap-2 bg-white hover:bg-app-cream transition-colors">
                    Add New Address <PlusIcon className="size-4" />
                </Link>
                <button onClick={() => { setStep("payment"); scrollTo(0, 0) }} disabled={!address.address || !address.city} className="w-full sm:w-auto px-6 py-3 bg-app-green text-white font-semibold rounded-xl hover:bg-app-green-light transition-colors disabled:opacity-50 flex items-center justify-center gap-2">
