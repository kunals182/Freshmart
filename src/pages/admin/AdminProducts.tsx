...
export default function AdminProducts() {
    const { products, markOutOfStock, currency } = useApp();
    ...
    const handleMarkOutOfStock = (id: string, name: string) => {
                                    ...
                                    <tr key={product._id} className="hover:bg-zinc-50/50 transition-colors">
                                        <td className="px-6 py-4">
                                            <div className="flex items-center gap-3">
                                                <img src={product.image} alt={product.name} className="size-12 rounded-lg object-cover bg-app-cream/10 border p-1" />
                                                <div>
                                                    <p className="font-semibold text-zinc-900">{product.name}</p>
                                                    <p className="text-xs text-zinc-500 capitalize">{product.category.replace("-", " ") || "Uncategorized"}</p>
                                                </div>
                                            </div>
                                        </td>
