...
export default function AdminProductForm() {
    ...
    const { products, addProduct, updateProduct, currency } = useApp();
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        // Simulate file upload or use default mockup image URL
        let finalImage = formData.image;
        if (imageFile) {
            finalImage = URL.createObjectURL(imageFile); // temporary blob URL
        } else if (!finalImage) {
            finalImage = "https://images.unsplash.com/photo-1542838132-92c53300491e?w=200"; // fallback
        }

        setTimeout(() => {
            if (isEdit && id) {
                updateProduct(id, { ...formData, image: finalImage });
            } else {
                ...
                {loading ? (
                    ...
                    <form onSubmit={handleSubmit} className="p-6 space-y-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                <label className="block text-sm font-medium text-zinc-700 mb-2">Price ({currency})</label>
                            ...
                            <div>
                                <label className="block text-sm font-medium text-zinc-700 mb-2">Original Price ({currency}) - Optional</label>
