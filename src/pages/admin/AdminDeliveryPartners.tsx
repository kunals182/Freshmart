...
export default function AdminDeliveryPartners() {
    const { partners, addPartner, togglePartnerActive } = useApp();
                        ...
                        <div key={p._id} className="bg-white rounded-2xl border border-app-border p-5 space-y-3 shadow-xs">
                            <div className="flex items-center justify-between">
                                <div className="flex items-center gap-3">
                                    <div className="size-10 rounded-full bg-app-green text-white flex-center">
                                        <span className="font-semibold text-sm">{p.name.charAt(0)}</span>
                                    </div>
                                    <div>
                            ...
                            <button onClick={() => toggleActive(p._id, p.isActive || false)} className={`w-full py-2 text-xs font-medium rounded-lg transition-colors ${p.isActive ? "bg-red-50 text-red-600 hover:bg-red-100" : "bg-green-50 text-green-600 hover:bg-green-100"}`}>
            ...
            {showForm && (
                <>
                    ...
                    <div className="fixed inset-0 z-50 flex-center p-4">
                        <form onSubmit={handleSubmit} className="bg-white rounded-2xl p-6 w-full max-w-lg animate-fade-in border border-app-border">
