...
export default function DeliveryDashboard() {
    ...
    // OTP modal
    const [otpModal, setOtpModal] = useState<string | null>(null); // holds orderId
    const [otp, setOtp] = useState("");
    const [submitting, setSubmitting] = useState(false);

    // Cancel modal
    const [cancelModal, setCancelModal] = useState<string | null>(null); // holds orderId
    const [cancelReason, setCancelReason] = useState("");

    // Active deliveries: Assigned, Packed, Out for Delivery
    // Completed: Delivered, Cancelled
    const filteredOrders = orders.filter(order => {
        const isActive = ["Assigned", "Packed", "Out for Delivery"].includes(order.status);
    ...
    const handleComplete = () => {
        ...
        const order = orders.find(o => o._id === otpModal);
        if (!order) {
            setSubmitting(false);
            return;
        }
        ...
        setTimeout(() => {
            if (otp === order.deliveryOtp) {
                updateOrderStatus(otpModal, "Delivered", "Delivered successfully by partner");
    ...
    const handleCancel = () => {
        ...
        setTimeout(() => {
            updateOrderStatus(cancelModal, "Cancelled", `Delivery cancelled: ${cancelReason}`);
        ...
        <div className="space-y-6 animate-fade-in">
            ...
            {loading ? (
                ...
                <div className="space-y-4">
                    {filteredOrders.map((order) => <DeliveryOrderCard key={order._id} order={order} tab={tab} handleUpdateStatus={handleUpdateStatus} setOtpModal={setOtpModal} setCancelModal={setCancelModal} />)}
                </div>
            )}
